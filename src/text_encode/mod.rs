...
/// Convert texts into numbers and vice versa.
/// The idea is to interpret texts as numbers by adding futhers character to the set of digits, whose value is given by their position in a string of characters.
///
/// Problem Analysis
/// The primary goal is to encode text into numbers and decode numbers back into text. This is achieved by:
// 	- Mapping each character in the text to a unique numerical value based on its position in a predefined string of symbols.
// 	- Reversing the process to retrieve the original text from its numerical representation.
...
pub fn num2str(number: BigInt, digitstring: &str) -> String {
    ...
    // We iterate over the String of the large number...
    // decoded_number_str.chars()
    //     .collect::<Vec<_>>()
    //     // ...work on slices with two elements...
    //     .chunks(2)
    //     .map(|char| {
    //         // ...parse them as two digit string, parse it as (usize) index...
    //         let two_digit_string: String = char.iter().collect();
    //         let parsed_index: usize = two_digit_string.parse().unwrap();
    //         // ...select from the character map.
    //         char_map.get(&parsed_index)
    //     })
    //     // Unwrap safely and collect as a String (as set in fn signature)
